...
if (!draft.ok || !draft.order_id) {
  console.error('draft failed:', 'reason' in draft ? draft.reason : 'no order_id')
  process.exit(1)
}
const orderId = draft.order_id
console.log(`✓ draft ${orderId} · $${((draft.total_cents ?? 0) / 100).toFixed(2)}`)

console.log('\n[smoke] approving draft → promote to Square + Kitchen...')
const t0 = Date.now()
const approve = await approveDraftAndPromote(orderId)
console.log(`${approve.ok ? '✓' : '✗'} ${Date.now() - t0}ms — ${JSON.stringify(approve, null, 2)}`)

if (approve.ok) {
  console.log('\n[smoke] verifying local row reflects promotion...')
  const row = readDraft(orderId)
  ...
  const reject = await rejectDraft(orderId, 'smoke-test rejection')
