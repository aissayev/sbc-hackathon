...
//
// These hit the sandbox over HTTP (covered by the team token, not Claude Max)
// — instant, free, no `claude -p` spend. Async only because of network I/O.
//
// Closes the marketing/social-management gap on the owner cockpit:
//
//   /inbox      WhatsApp + Instagram thread inbox (oldest unanswered first)
//   /reviews    Recent Google Business reviews + 1-tap reply
//   /campaigns  Active campaigns + metrics + pause/double-down
...
async function campaignsReply(): Promise<BotReply> {
  ...
  for (const c of active) {
    ...
    rows.push(`\ud83d\udce3 ${trunc(name, 28)}  $${spend.toFixed(0)}/$${budget.toFixed(0)}  ${leads}L  CTR ${ctr}`)
