...
import {
  CareersApplyProvider,
  ApplyButton,
  ApplyOtherButton,
} from '@/components/careers/application-form'
import { Phone } from 'lucide-react'

export const metadata: Metadata = {
  title: `Careers — join the HappyCake team | ${BRAND.name}`,
  description:
    'Join the HappyCake team in Sugar Land. We hire kind, careful people who love feeding their neighbours. Apply in 60 seconds — our team replies within a few business days.',
  ...
  return (
    <CareersApplyProvider>
      <main>
        ...
        {/* Open roles — each card has an in-app Apply button that opens
            the modal with the role pre-selected. No more mailto: links. */}
        <section className="container pb-20 md:pb-28">
          <div className="grid gap-8 md:grid-cols-3">
            {CAREERS_ROLES.map((r) => (
              <div key={r.slug} className="bakery-card p-6 md:p-7 flex flex-col">
                ...
                <ApplyButton role={r.slug} className="mt-6" />
              </div>
            ))}
          </div>
