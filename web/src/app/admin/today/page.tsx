  ...
  return (
    <div>
      ...
      <section className="mt-10">
        ...
        {orders.length === 0 ? (
          <div className="rounded-md bg-cream-100 p-6 text-sm text-happy-900/70">
            No orders yet today. As soon as one comes in through chat, web, WhatsApp, or Instagram,
            it'll show up here for approval.
          </div>
        ) : (
          <ul className="divide-y divide-happy-700/10 rounded-md border border-happy-700/15 bg-white">
            {orders.slice(0, 8).map((o) => (
              <li key={o.id} className="p-4 flex items-center gap-3 justify-between flex-wrap">
                <div>
                  <Link href={`/admin/orders/${o.id}`} className="font-medium text-happy-900 hover:text-happy-700">
                    #{o.id.slice(-8)}
                  </Link>
      ...
      {!report && (
        <p className="mt-8 text-xs text-happy-900/60">
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className={`rounded-md p-5 border ${highlight ? 'bg-happy-200 border-happy-700/30' : 'bg-cream-100 border-happy-700/10'}`}>
      <div className="text-xs uppercase tracking-[0.16em] text-happy-900/60">{label}</div>
      <div className="text-3xl font-display text-happy-900 mt-1">{value}</div>
    </div>
  )
}
