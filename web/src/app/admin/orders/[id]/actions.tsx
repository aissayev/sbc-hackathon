...
export function OrderActions({ orderId, status }: { orderId: string; status: string }) {
  ...
  if (!['draft'].includes(status)) {
    return <p className="mt-2 text-sm text-cocoa-900/60">Already moved past approval. No actions to take.</p>
  }
  ...
  return (
    <div className="mt-3 space-y-3">
      ...
      <details className="rounded-md border border-cocoa-700/15">
        <summary className="cursor-pointer p-3 text-sm text-cocoa-900">Reject this order</summary>
      ...
      {error && (
        <p className="text-xs text-berry bg-berry/10 rounded-md p-2" role="alert">
          {error}
        </p>
      )}
    </div>
