...
export function OrderForm({ products }: { products: Product[] }) {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-8 lg:grid-cols-[1fr_360px]">
        ...
        <section>
          ...
          <div className="mt-5 space-y-3">
            {fields.map((field, idx) => {
              ...
              return (
                <div key={field.id} className="rounded-md border border-happy-700/15 bg-white p-4">
                  <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-end">
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <select
                            {...field}
                            id={`item-${idx}`}
                            className="mt-1 flex h-11 w-full rounded-md border border-happy-700/20 bg-white px-3 text-sm"
                          >
                            {products.map((p) => (
                    ...
                    <div>
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <div className="mt-1 inline-flex items-center rounded-md border border-happy-700/20 bg-white">
                            <button
                              type="button"
                              className="h-11 w-10 text-happy-700 hover:bg-cream-100 disabled:opacity-30"
                            ...
                            <input
                              ...
                              className="h-11 w-12 text-center bg-transparent border-x border-happy-700/15 text-sm"
                            />
                            <button
                              type="button"
                              className="h-11 w-10 text-happy-700 hover:bg-cream-100"
                  ...
                  {product?.description && (
                    <p className="mt-3 text-sm text-happy-900/70">{product.description}</p>
                  )}
                  <div className="mt-3 flex items-center gap-2 text-xs text-happy-900/60">
        ...
        <section>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              ...
              {maxLead > 0 && (
                <p className="mt-1 text-xs text-happy-900/60">
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        value === mode
                          ? 'border-happy-700 bg-happy-700 text-cream-50'
                          : 'border-happy-700/20 bg-white text-happy-900 hover:bg-cream-100',
                      )}
                    >
                      {mode}
                    </button>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              ...
              {form.formState.errors.customer_name && (
                <p className="mt-1 text-xs text-coral">{form.formState.errors.customer_name.message}</p>
              )}
            </div>
            <div>
              ...
              {form.formState.errors.customer_phone && (
                <p className="mt-1 text-xs text-coral">{form.formState.errors.customer_phone.message}</p>
              )}
            </div>
          </div>
          <div className="mt-4">
      ...
      <aside className="lg:sticky lg:top-28 self-start rounded-lg bg-cream-100 border border-happy-700/15 p-6">
        <p className="eyebrow">Order summary</p>
        <h2 className="display-h3 mt-1">Your basket</h2>
        <ul className="mt-4 divide-y divide-happy-700/15">
          {watchItems.map((it, i) => {
            const p = products.find((x) => x.id === it.product_id)
            if (!p) return null
            return (
              <li key={i} className="py-3 flex items-center justify-between gap-3 text-sm">
                <span className="text-happy-900">
                  {p.name}
                  <span className="text-happy-900/60"> × {it.quantity}</span>
                </span>
                <span className="font-medium text-happy-900">
        ...
        <div className="mt-4 border-t border-happy-700/15 pt-4 flex items-end justify-between">
          <span className="text-xs uppercase tracking-[0.16em] text-happy-900/60">Subtotal</span>
          <span className="text-2xl font-medium text-happy-900">{fmtUsd(total)}</span>
        </div>
        ...
        {error && (
          <p className="mt-3 text-sm text-coral bg-coral/10 rounded-md p-3" role="alert">
            {error}
          </p>
        )}
        ...
        <p className="mt-3 text-xs text-happy-900/60">
